...
export default function AskAbhi() {
 ...
 // Colours that switch with theme
 const C = isDark ? {
  panel:   '#080808',
  border:   '#1e1e1e',
  borderHov: '#2e2e2e',
  ...
  btnInact:  '#1e1e1e',
  btnTxtIn:  '#4a4a4a',
  dotClose:  'rgba(255,255,255,0.8)',
  dotCloseBg: 'rgba(0,0,0,0.2)',
  ...
  borderHov: 'rgba(255,77,0,0.3)',
  msgBg:   '#f5f0e8',
  msgBorder: 'rgba(0,0,0,0.08)',
  text:    '#080808',
  muted:   '#6a6060',
  dim:    '#9a9090',
  strip:   '#f0ebe2',
  stripBdr:  'rgba(0,0,0,0.07)',
  input:   '#f5f0e8',
  inputBdr:  'rgba(0,0,0,0.12)',
  userText:  '#ffffff',
  btnInact:  'rgba(0,0,0,0.08)',
  btnTxtIn:  '#9a9090',
  dotClose:  '#080808',
  dotCloseBg: 'rgba(0,0,0,0.12)',
  avatarBg:  'rgba(0,0,0,0.2)',
  starterBdr: 'rgba(0,0,0,0.12)',
  starterTxt: '#6a6060',
  shadow:   '0 24px 80px rgba(0,0,0,0.18), 0 2px 8px rgba(0,0,0,0.1)',
 }
 ...
 const send = async (text: string) => {
  ...
  try {
   ...
   setMessages(m => [...m, { role: 'assistant', content: data.reply || data.error || 'WhatsApp me: +971 52 677 6884 ' }])
  } catch {
   setMessages(m => [...m, { role: 'assistant', content: 'Connection issue. WhatsApp me: +971 52 677 6884 ' }])
  ...
  <>
   ...
   <div style={{
     ...
     <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
      <div style={{ position: 'relative' }}>
       <div style={{
        ...
        background: C.avatarBg, border: '2px solid rgba(255,255,255,0.2)',
      ...
      <div>
       ...
       <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.75)', fontFamily: 'monospace', letterSpacing: '0.5px' }}>Senior Creative Designer · Dubai</div>
      </div>
     </div>
     <button onClick={() => setOpen(false)} style={{
      background: C.dotCloseBg, border: 'none', color: C.dotClose,
    ...
    <div style={{
     ...
     scrollbarWidth: 'none', background: C.panel,
    }}>
     {messages.length === 0 && (
      <div>
       <div style={{
        background: C.msgBg, border: `1px solid ${C.msgBorder}`,
        borderRadius: '4px 14px 14px 14px', padding: '13px 15px', maxWidth: '90%',
       }}>
        <p style={{ fontFamily: 'var(--font-body,DM Sans,sans-serif)', fontSize: '13.5px', color: C.text, lineHeight: 1.7, margin: 0 }}>
         I'm Abhi. Tell me what you&apos;re building — I&apos;m all ears.
        </p>
       </div>
       <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', marginTop: '14px' }}>
        {STARTERS.map(s => (
         <button key={s} onClick={() => send(s)} style={{
          background: 'transparent', border: `1px solid ${C.starterBdr}`,
          borderRadius: '8px', padding: '9px 13px', textAlign: 'left',
          color: C.starterTxt, fontFamily: 'var(--font-body,DM Sans,sans-serif)',
          fontSize: '12px', cursor: 'pointer', transition: 'all .2s ease', lineHeight: 1.4,
         }}
         onMouseEnter={e => { e.currentTarget.style.borderColor = '#ff4d00'; e.currentTarget.style.color = '#ff4d00'; e.currentTarget.style.background = 'rgba(255,77,0,0.06)' }}
         onMouseLeave={e => { e.currentTarget.style.borderColor = C.starterBdr; e.currentTarget.style.color = C.starterTxt; e.currentTarget.style.background = 'transparent' }}
         >{s}</button>
        ))}
       </div>
      </div>
     ...
     {messages.map((m, i) => (
      <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
       <div style={{
        maxWidth: '86%',
        background: m.role === 'user' ? '#ff4d00' : C.msgBg,
        border: m.role === 'user' ? 'none' : `1px solid ${C.msgBorder}`,
    ...
    {/* WhatsApp strip */}
    <div style={{
     padding: '8px 16px', background: C.strip,
     borderTop: `1px solid ${C.stripBdr}`,
     ...
     <a href="https://wa.me/971526776884?text=Hi%20Abhi%2C%20I%20visited%20your%20portfolio%20and%20would%20like%20to%20discuss%20a%20project."
      ...
      style={{ fontFamily: 'monospace', fontSize: '9px', letterSpacing: '0.8px', color: C.dim, textDecoration: 'none' }}>
    ...
    <div style={{
     ...
     <input
      ...
      disabled={loading}
      style={{
       flex: 1, background: C.input, border: `1px solid ${C.inputBdr}`,
       borderRadius: '8px', padding: '10px 13px', color: C.text,
     ...
     <button onClick={() => send(input)} disabled={!input.trim() || loading} style={{
      background: input.trim() && !loading ? '#ff4d00' : C.btnInact,
      ...
      <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
       <path d="M1.5 7.5H13.5M13.5 7.5L7.5 1.5M13.5 7.5L7.5 13.5"
        stroke={input.trim() && !loading ? '#ffffff' : C.btnTxtIn}
   ...
   <button
    ...
    style={{
     ...
     background: open ? C.btnInact : '#ff4d00',
   ...
   >
    {open ? (
     <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <path d="M2 2L14 14M14 2L2 14" stroke={C.muted} strokeWidth="2" strokeLinecap="round"/>
     </svg>
    ) : (
