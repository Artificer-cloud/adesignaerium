        ...
        <div style={{ position: 'relative', zIndex: 1, padding: 'clamp(40px,6vh,80px) clamp(20px,6vw,80px)', maxWidth: '1400px', margin: '0 auto', width: '100%' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '24px' }}>
            {project.category.map((cat: string) => (
              <span key={cat} style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '2.5px', color: 'var(--orange)', background: 'rgba(255,77,0,0.12)', border: '1px solid rgba(255,77,0,0.35)', padding: '5px 14px', borderRadius: '2px' }}>{cat.toUpperCase()}</span>
            ))}
          </div>
      ...
      <section style={{ padding: '0 clamp(20px,6vw,80px) clamp(60px,8vh,100px)', maxWidth: '860px', margin: '0 auto' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
          {project.tags.map((tag: string) => (
            <span key={tag} style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '1.5px', color: 'var(--dim)', border: '1px solid var(--border)', padding: '6px 14px', borderRadius: '100px' }}>{tag}</span>
          ))}
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,260px),1fr))', gap: '12px' }}>
            {others.map((other: Project) => (
              <Link key={other.id} href={'/work/' + other.id} className="project-card"
                ...
                {other.coverImage && (
                  <>
                    <Image src={other.coverImage} alt={other.title} fill sizes="33vw" style={{ objectFit: 'cover', opacity: 0.45 }} />
