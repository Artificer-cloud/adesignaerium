...
export default function ContactPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    window.location.href = `mailto:abhijeethpiyush4@gmail.com?subject=${encodeURIComponent(`Portfolio Enquiry from ${data.get('name')}`)}&body=${encodeURIComponent(`From: ${data.get('name')} (${data.get('email')})\n\n${data.get('message')}`)}`
    setSent(true)
  }
          ...
          <div className="contact-form-col">
            {sent ? (
              ...
              <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'24px' }}>
                {[{name:'name',label:'YOUR NAME',type:'text',ph:'e.g. Sarah Al Maktoum'},{name:'email',label:'EMAIL ADDRESS',type:'email',ph:'hello@yourbrand.com'}].map(f=>(
                  <div key={f.name}>
                    ...
                    <input name={f.name} required type={f.type} placeholder={f.ph} style={inputStyle}
                ...
                <button type="submit" style={{ fontFamily:'Clash Display,Arial Black,sans-serif', fontWeight:600, fontSize:'13px', letterSpacing:'2px', textTransform:'uppercase', background:'var(--orange)', color:'var(--ink)', padding:'16px', borderRadius:'2px', border:'none', cursor:'pointer', width:'100%' }}>
                  Send Message →
                </button>
              </form>
            )}
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
                {[{l:'Email',v:'abhijeethpiyush4@gmail.com',h:'mailto:abhijeethpiyush4@gmail.com'},{l:'Phone',v:'+971 52 677 6884',h:'tel:+971526776884'},{l:'WhatsApp',v:'+971 52 677 6884',h:'https://wa.me/971526776884'},{l:'Location',v:'Dubai, UAE',h:''}].map(({l,v,h})=>(
                  <div key={l}>
                    ...
                    {h ? <a href={h} target={h.startsWith('http')?'_blank':undefined} rel={h.startsWith('http')?'noopener noreferrer':undefined} style={{ fontFamily:'var(--font-body)', fontSize:'15px', color:'var(--bone)', wordBreak:'break-all' }}>{v} ↗</a>
            ...
            <div style={{ marginBottom:'36px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px' }}>
                {[{l:'Behance',h:'https://behance.net/abhijeeth-subhash'},{l:'Dribbble',h:'https://dribbble.com/Artificer_666'},{l:'LinkedIn',h:'https://linkedin.com/in/abhijeethsubhash'},{l:'@wonderartmedia',h:'https://instagram.com/wonderartmedia'}].map(({l,h})=>(
                  <a key={l} href={h} target="_blank" rel="noopener noreferrer" style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'1px', color:'var(--muted)', border:'1px solid var(--border)', padding:'10px 14px', borderRadius:'2px', display:'flex', alignItems:'center', justifyContent:'space-between', textDecoration:'none' }}>
