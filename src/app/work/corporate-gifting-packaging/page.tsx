...
export const metadata = { title: 'Packaging & Print — Case Study | Abhijeeth Subhash' }

export default function CorporateGiftingPackagingPage() {
  const project = projects.find(p => p.id === 'corporate-gifting-packaging')!
  const others = projects.filter(p => p.id !== 'corporate-gifting-packaging').slice(0, 3)
  return (
    <main style={{ paddingTop: '80px', minHeight: '100vh' }}>
      ...
      <section style={{ position: 'relative', minHeight: 'clamp(460px,60vh,700px)', display: 'flex', alignItems: 'flex-end', overflow: 'hidden', background: project.color || '#111' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: 'clamp(40px,6vh,80px) clamp(20px,6vw,80px)', maxWidth: '1400px', margin: '0 auto', width: '100%' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '24px' }}>
            {project.category.map((cat: string) => <span key={cat} style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '2.5px', color: 'var(--orange)', background: 'rgba(255,77,0,0.12)', border: '1px solid rgba(255,77,0,0.35)', padding: '5px 14px', borderRadius: '2px' }}>{cat.toUpperCase()}</span>)}
          </div>
      ...
      <section style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ padding: 'clamp(24px,4vh,48px) clamp(20px,6vw,80px)', maxWidth: '1400px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 'clamp(24px,4vw,48px)' }}>
          {[{label:'Client',value:project.client},{label:'Year',value:project.year},{label:'Role',value:project.role.join(', ')},{label:'Scope',value:project.tags.slice(0,3).join(' · ')}].map(({label,value}: any) => (
            <div key={label}>
      ...
      <section style={{ padding: 'clamp(60px,8vh,100px) clamp(20px,6vw,80px)', maxWidth: '860px', margin: '0 auto' }}>
        ...
        {project.longDescription.split('\n\n').map((para: string, i: number) => <p key={i} style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(15px,1.8vw,19px)', color: 'var(--muted)', lineHeight: 1.85, marginBottom: '1.6rem' }}>{para}</p>)}
      </section>
      <section style={{ padding: '0 clamp(20px,6vw,80px) clamp(60px,8vh,100px)', maxWidth: '860px', margin: '0 auto' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
          {project.tags.map((tag: string) => <span key={tag} style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '1.5px', color: 'var(--dim)', border: '1px solid var(--border)', padding: '6px 14px', borderRadius: '100px' }}>{tag}</span>)}
        </div>
      </section>
