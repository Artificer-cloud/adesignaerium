...
/* ─────────────────────────────────────────────────────────────────
   FONT CYCLE ANIMATION
   Cycles through fonts before landing on Clash Display
───────────────────────────────────────────────────────────────── */
const FONT_CYCLE = [
  ...
  return (
    <div style={{ opacity:mounted?1:0, transform:mounted?'translateY(0)':'translateY(36px)', transition:'opacity .5s .2s, transform .5s .2s' }}>
      ...
      <h1 style={{ lineHeight:.86, margin:0, position:'relative' }}>
        ...
        {/* Orange underline draws in on arrival */}
        <span style={{
  ...
  return (
    <main>
      ...
      <section ref={heroRef} onMouseMove={handleHeroMouse}
        ...
        {/* Two-column content */}
        <div style={{ display:'grid', gridTemplateColumns:'1fr clamp(200px,30vw,420px)', gap:'clamp(24px,4vw,60px)', padding:'clamp(90px,12vh,130px) clamp(20px,6vw,80px) clamp(60px,8vh,100px)', maxWidth:'1400px', margin:'0 auto', width:'100%', alignItems:'center' }}>
          ...
          <div style={{ position:'relative', width:'100%', aspectRatio:'3/4', opacity:mounted?1:0, transform:mounted?'translateY(0)':'translateY(40px)', transition:'all 1.1s cubic-bezier(.23,1,.32,1) .6s' }}>
            {/* Doodle ring */}
            <svg aria-hidden className="animate-spin-slow" style={{ position:'absolute', inset:'-8%', width:'116%', height:'116%', opacity:.12, pointerEvents:'none', zIndex:0 }} viewBox="0 0 400 400">
            ...
            <div style={{ position:'relative', width:'100%', height:'100%', borderRadius:'8px', overflow:'hidden', background:'rgba(255,255,255,0.04)', border:'1px dashed rgba(255,255,255,0.1)', zIndex:1 }}>
              <Image src="/images/hero-portrait.png" alt="Abhijeeth Subhash — Senior Creative Designer Dubai" fill style={{ objectFit:'cover', objectPosition:'center top' }} priority onError={() => {}} />
              {/* Placeholder hint */}
              <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'6px', pointerEvents:'none', zIndex:2 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'2px', color:'rgba(255,255,255,0.15)', textAlign:'center', lineHeight:2 }}>
                  HERO PORTRAIT<br/>
                  <span style={{ fontSize:'9px' }}>hero-portrait.png · 800×1067px</span>
                </span>
              </div>
              {/* Bottom fade */}
              <div style={{ position:'absolute', bottom:0, left:0, right:0, height:'35%', background:'linear-gradient(transparent,var(--ink))', zIndex:3, pointerEvents:'none' }} />
            </div>
            {/* Floating tag */}
            <div className="animate-float" style={{ position:'absolute', bottom:'12%', left:'-8%', zIndex:4, background:'rgba(255,77,0,0.12)', border:'1px solid rgba(255,77,0,0.3)', borderRadius:'100px', padding:'6px 16px', fontFamily:'var(--font-mono)', fontSize:'9px', letterSpacing:'2px', color:'var(--orange)' }}>
              CREATIVE DIRECTOR
            </div>
          </div>
        </div>
