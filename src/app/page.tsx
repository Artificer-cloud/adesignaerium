...
/* ── FONT CYCLING NAME ANIMATION ────────────────────────────────── */
// Cycles through different fonts before landing on Clash Display
const FONT_CYCLE = [
  ...
  { font: 'Impact, Arial Black, sans-serif',     weight: '900', style: 'normal',  color: 'var(--muted)' },
  { font: 'Clash Display, Arial Black, sans-serif', weight: '700', style: 'normal', color: 'var(--bone)' }, // FINAL
]
...
function useFontCycle(trigger: boolean) {
  ...
  useEffect(() => {
    ...
    const start = setTimeout(() => {
      setVisible(true)
      let i = 0
      const speed = [60,55,50,55,50,60,50,55,60,0] // ms per frame, last=0 means stop
      const step = () => {
        setIdx(i)
        if (i < FONT_CYCLE.length - 1) {
          i++
          setTimeout(step, speed[i] ?? 60)
        } else {
          setDone(true)
        }
      }
  ...
  return { fontStyle: FONT_CYCLE[idx], done, visible }
}

/* ── MAGNETIC BUTTON ────────────────────────────────────────────── */
function Magnetic({ children, href, style }: {
  ...
  const inner = (
    <div ref={r} onMouseMove={mv} onMouseLeave={lv}
      style={{ transition:'transform .4s cubic-bezier(.23,1,.32,1)', display:'inline-block' }}>
      {children}
    </div>
  )
...
function SectionArt({ v = 0 }: { v?: number }) {
  const arts = [
    // Hero — design tools
    <svg key={0} className="section-art" viewBox="0 0 1400 800" preserveAspectRatio="xMidYMid slice" style={{opacity:.04}}>
      <circle cx="1200" cy="150" r="110" fill="none" stroke="#ff4d00" strokeWidth="1" strokeDasharray="8 6"/>
      <circle cx="1200" cy="150" r="65"  fill="none" stroke="#ff4d00" strokeWidth=".5" strokeDasharray="3 7"/>
      ...
      {['x','x','x','x'].map((_,i) => <rect key={i} x={80+i*30} y={640} width="24" height="24" rx="2" stroke={i===0?'#ff4d00':'#ede8dd'} strokeWidth="1" fill="none"/>)}
      {Array.from({length:22}).map((_,i) => <line key={i} x1={220+i*34} y1={740} x2={220+i*34} y2={i%5===0?718:730} stroke="#ede8dd" strokeWidth=".6"/>)}
    </svg>,
    // Work — grid layout
    <svg key={1} className="section-art" viewBox="0 0 1400 600" preserveAspectRatio="xMidYMid slice" style={{opacity:.04}}>
      ...
      <rect x="500" y="60" width="300" height="80" rx="4" stroke="#ede8dd" strokeWidth=".8" fill="none"/>
      <rect x="500" y="160" width="300" height="80" rx="4" stroke="#ede8dd" strokeWidth=".8" fill="none"/>
    </svg>,
    // Skills — ghost A letter
    <svg key={2} className="section-art" viewBox="0 0 1400 500" preserveAspectRatio="xMidYMid slice" style={{opacity:.035}}>
    ...
    // CTA — diagonals + stars
    <svg key={3} className="section-art" viewBox="0 0 1400 500" preserveAspectRatio="xMidYMid slice" style={{opacity:.04}}>
      {Array.from({length:14}).map((_,i) => (
        <line key={i} x1={-80+i*110} y1={0} x2={i*110-600} y2={600} stroke={i%2===0?'#ff4d00':'#ede8dd'} strokeWidth=".6"/>
      ))}
      {[[220,250],[700,160],[1180,300]].map(([x,y],i) => (
        <g key={i}>
          ...
          <line x1={x-15} y1={y-15} x2={x+15} y2={y+15} stroke="#ff4d00" strokeWidth=".8"/>
          <line x1={x+15} y1={y-15} x2={x-15} y2={y+15} stroke="#ff4d00" strokeWidth=".8"/>
        </g>
      ))}
    </svg>,
...
const TAGLINES = [
  ...
  "Not trends. Timeless impact.",
  "Every pixel has a purpose.",
  "Where ideas become visual worlds.",
]

/* ── FONT CYCLE NAME COMPONENT ──────────────────────────────────── */
function FontCycleName({ mounted }: { mounted: boolean }) {
  const { fontStyle, done, visible } = useFontCycle(mounted)
    ...
    <div style={{opacity:mounted?1:0,transform:mounted?'translateY(0)':'translateY(36px)',transition:'opacity .5s .2s, transform .5s .2s'}}>
      ...
      <h1 style={{lineHeight:.86,margin:0,position:'relative'}}>
        <span style={{
          ...
          opacity: visible ? 1 : 0,
          /* Blur clears as font settles */
          filter: done ? 'blur(0px)' : 'blur(0.5px)',
          transition: done
            ? 'font-family .15s ease, letter-spacing .5s cubic-bezier(.23,1,.32,1), color .2s ease, filter .3s ease'
            : 'font-family .08s ease, color .08s ease, filter .1s ease',
        }}>
          Abhi.
        </span>

        {/* Underline that draws in when done */}
        <span style={{
          position:'absolute',
          bottom:'-4px',left:0,
          height:'3px',
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const fn = (e: MouseEvent) => {
      ...
      hero.querySelectorAll<SVGElement>('.pl').forEach((el, i) => {
        const d = (i + 1) * .3
  ...
  return (
    <main>
      {/* ── HERO ──────────────────────────────────────────────────── */}
      <section ref={heroRef} style={{minHeight:'100svh',background:'var(--ink)',position:'relative',overflow:'hidden',display:'flex',alignItems:'center'}}>
        <SectionArt v={0}/>
        {/* Parallax circles */}
        <svg className="pl" style={{position:'absolute',inset:0,width:'100%',height:'100%',opacity:.06,pointerEvents:'none',transition:'transform .08s ease-out'}}
          viewBox="0 0 1400 900" preserveAspectRatio="xMidYMid slice">
          <circle cx="1250" cy="140" r="100" fill="none" stroke="#ff4d00" strokeWidth="1" strokeDasharray="7 5"/>
          <circle cx="1250" cy="140" r="58"  fill="none" stroke="#ff4d00" strokeWidth=".5" strokeDasharray="3 7"/>
        </svg>
        <svg className="pl" style={{position:'absolute',inset:0,width:'100%',height:'100%',opacity:.04,pointerEvents:'none',transition:'transform .12s ease-out'}}
          viewBox="0 0 1400 900" preserveAspectRatio="xMidYMid slice">
        ...
        {/* Spinning badge */}
        <div className="animate-spin-slow" style={{position:'absolute',top:'clamp(70px,10vh,90px)',right:'clamp(16px,4vw,56px)',width:'80px',height:'80px',opacity:.2}}>
        ...
        <div style={{
          display:'grid',
          gridTemplateColumns:'1fr clamp(200px,30vw,420px)',
          ...
          <div>
            ...
            <div style={{
              ...
              fontFamily:'var(--font-mono)',fontSize:'10px',letterSpacing:'2px',color:'var(--muted)',
            ...
            {/* Greeting + Name — Font Cycle Animation */}
            <FontCycleName mounted={mounted} />

            {/* Rotating tagline */}
            <div style={{
              height:'clamp(28px,3.5vw,44px)',overflow:'hidden',marginTop:'16px',
              opacity:mounted?1:0,transition:'opacity .9s cubic-bezier(.23,1,.32,1) .55s',
            }}>
              <p style={{
            ...
            {/* Sub description */}
            <p style={{
              fontFamily:'var(--font-body)',fontSize:'clamp(14px,1.6vw,18px)',
              color:'var(--muted)',lineHeight:1.7,marginTop:'20px',
              maxWidth:'520px',
              opacity:mounted?1:0,transform:mounted?'translateY(0)':'translateY(24px)',
              transition:'all .9s cubic-bezier(.23,1,.32,1) .65s',
            }}>
              Senior Creative Designer · 7+ years turning brands into visual worlds.
              Based in <span style={{color:'var(--bone)'}}>Dubai, UAE</span>.
            </p>
            ...
            <div style={{
              ...
              opacity:mounted?1:0,transform:mounted?'translateY(0)':'translateY(24px)',
              transition:'all .9s cubic-bezier(.23,1,.32,1) .78s',
            }}>
              <Magnetic href="/work">
                <span style={{display:'block',background:'var(--orange)',color:'var(--ink)',fontFamily:'Clash Display,sans-serif',fontWeight:600,fontSize:'12px',letterSpacing:'2px',textTransform:'uppercase',padding:'clamp(12px,1.5vh,16px) clamp(24px,3vw,40px)',borderRadius:'2px'}}>
                  View Work ↗
                </span>
              </Magnetic>
              <Magnetic href="/contact">
                <span style={{display:'block',border:'1px solid rgba(255,255,255,0.12)',color:'var(--bone)',fontFamily:'var(--font-mono)',fontSize:'11px',letterSpacing:'2px',textTransform:'uppercase',padding:'clamp(12px,1.5vh,16px) clamp(24px,3vw,40px)',borderRadius:'2px'}}>
                  Let&apos;s Talk
                </span>
              </Magnetic>
            </div>
            ...
            {/* Stats */}
            <div style={{
              display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:0,marginTop:'clamp(40px,6vh,72px)',
              borderTop:'1px solid var(--border)',paddingTop:'24px',maxWidth:'480px',
              opacity:mounted?1:0,transition:'opacity .9s cubic-bezier(.23,1,.32,1) 1.1s',
            }}>
              {STATS.map(({n,l}) => (
                <div key={l}>
                  <div style={{fontFamily:'Clash Display,sans-serif',fontWeight:700,fontSize:'clamp(22px,3.5vw,44px)',color:'var(--orange)',lineHeight:1}}>{n}</div>
          ...
          {/* RIGHT — Photo placeholder (transparent PNG) */}
          <div style={{
            position:'relative',width:'100%',aspectRatio:'3/4',
            opacity:mounted?1:0,transform:mounted?'translateY(0)':'translateY(40px)',
            transition:'all 1.1s cubic-bezier(.23,1,.32,1) .6s',
          }}>
            {/* Doodle ring behind image */}
            <svg style={{position:'absolute',inset:'-8%',width:'116%',height:'116%',opacity:.12,pointerEvents:'none',zIndex:0,animation:'spinSlow 20s linear infinite'}}
              viewBox="0 0 400 400">
              <circle cx="200" cy="200" r="190" fill="none" stroke="#ff4d00" strokeWidth="1" strokeDasharray="8 8"/>
            </svg>
            {/* Image container */}
            <div style={{position:'relative',width:'100%',height:'100%',borderRadius:'8px',overflow:'hidden',background:'rgba(255,255,255,0.04)',border:'1px dashed rgba(255,255,255,0.1)',zIndex:1}}>
              <Image
              ...
              />
              {/* Placeholder text shown until real image added */}
              <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'8px',zIndex:2}}>
                <span style={{fontFamily:'var(--font-mono)',fontSize:'10px',letterSpacing:'2px',color:'rgba(255,255,255,0.2)'}}>HERO PORTRAIT</span>
                <span style={{fontFamily:'var(--font-mono)',fontSize:'9px',letterSpacing:'1px',color:'rgba(255,255,255,0.1)'}}>hero-portrait.png</span>
                <span style={{fontFamily:'var(--font-mono)',fontSize:'9px',letterSpacing:'1px',color:'rgba(255,255,255,0.1)'}}>transparent PNG · 800×1067px</span>
              </div>
              {/* Bottom fade */}
              <div style={{position:'absolute',bottom:0,left:0,right:0,height:'35%',background:'linear-gradient(to bottom,transparent,var(--ink))',zIndex:3,pointerEvents:'none'}}/>
            </div>
            {/* Floating tag */}
            <div className="animate-float" style={{
              position:'absolute',bottom:'12%',left:'-8%',zIndex:4,
              background:'rgba(255,77,0,0.12)',border:'1px solid rgba(255,77,0,0.3)',
              borderRadius:'100px',padding:'6px 16px',
              fontFamily:'var(--font-mono)',fontSize:'9px',letterSpacing:'2px',color:'var(--orange)',
            }}>
              CREATIVE DIRECTOR
            </div>
          </div>
        ...
        {/* Scroll hint */}
        <div style={{position:'absolute',bottom:'20px',left:'50%',transform:'translateX(-50%)',display:'flex',flexDirection:'column',alignItems:'center',gap:'6px',opacity:mounted?.35:0,transition:'opacity 1s 1.6s'}}>
      ...
      {/* ── TICKER ────────────────────────────────────────────────── */}
      <div style={{background:'var(--orange)',overflow:'hidden',height:'34px',display:'flex',alignItems:'center'}}>
      ...
      <section style={{padding:'clamp(60px,8vh,100px) clamp(20px,6vw,80px)',maxWidth:'1400px',margin:'0 auto',position:'relative'}}>
        ...
        <div className="reveal" style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',marginBottom:'48px',flexWrap:'wrap',gap:'16px'}}>
          <div>
            ...
            <h2 style={{fontFamily:'Clash Display,sans-serif',fontWeight:700,fontSize:'clamp(32px,7vw,88px)',letterSpacing:'-3px',color:'var(--bone)',lineHeight:.88}}>
              THE<br/>
              <span style={{fontFamily:'Cormorant Garamond,serif',fontStyle:'italic',fontWeight:600,color:'var(--orange)'}}>Studio Wall</span>
            </h2>
          </div>
          ...
          {FEATURED.map((p,i) => (
            <Link key={p.id} href={`/work/${p.id}`} className="project-card"
              ...
              <svg style={{position:'absolute',inset:0,width:'100%',height:'100%',opacity:.06,pointerEvents:'none'}} viewBox="0 0 400 400" preserveAspectRatio="xMidYMid slice">
                ...
                <path d="M10,10 Q80,0 130,50" fill="none" stroke="#ede8dd" strokeWidth=".8" strokeDasharray="4 4"/>
              </svg>
              ...
              <h3 style={{fontFamily:'Clash Display,sans-serif',fontWeight:700,fontSize:'clamp(24px,4vw,52px)',letterSpacing:'-2px',color:'var(--bone)',lineHeight:.9}}>{p.title}</h3>
            </Link>
          ))}
        </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,200px),1fr))',border:'1px solid var(--border)'}}>
            {SKILLS.map(({n,s},i) => (
              <div key={n} className="reveal"
                style={{padding:'clamp(20px,2.5vw,28px)',borderRight:'1px solid var(--border)',borderBottom:'1px solid var(--border)',transition:'background .3s',transitionDelay:`${i*.06}s`,cursor:'default'}}
                onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background='rgba(255,77,0,0.04)'}}
                onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background='transparent'}}
              >
                <div style={{fontFamily:'Clash Display,sans-serif',fontWeight:600,fontSize:'clamp(16px,1.8vw,20px)',color:'var(--bone)',letterSpacing:'-.5px'}}>{n}</div>
      ...
      <section style={{padding:'clamp(80px,10vh,120px) clamp(20px,6vw,80px)',maxWidth:'1400px',margin:'0 auto',position:'relative',overflow:'hidden'}}>
        ...
        <h2 className="reveal" style={{fontFamily:'Clash Display,sans-serif',fontWeight:700,fontSize:'clamp(40px,9vw,130px)',letterSpacing:'-4px',lineHeight:.88,color:'var(--bone)'}}>
          LET&apos;S MAKE<br/>
          <span style={{fontFamily:'Cormorant Garamond,serif',fontStyle:'italic',fontWeight:600,color:'var(--orange)',letterSpacing:'-3px'}}>Something</span><br/>
          LOUD.
        </h2>
        <div className="reveal" style={{marginTop:'48px',display:'flex',gap:'14px',flexWrap:'wrap',alignItems:'center'}}>
          <Magnetic href="/contact">
            <span style={{display:'block',background:'var(--orange)',color:'var(--ink)',fontFamily:'Clash Display,sans-serif',fontWeight:600,fontSize:'12px',letterSpacing:'2px',textTransform:'uppercase',padding:'clamp(12px,1.5vh,16px) clamp(28px,4vw,44px)',borderRadius:'2px'}}>Email Me →</span>
          </Magnetic>
          <a href="https://wa.me/971526776884" target="_blank" rel="noopener noreferrer" className="hover-line"
            style={{fontFamily:'var(--font-mono)',fontSize:'11px',letterSpacing:'2px',color:'var(--muted)',textTransform:'uppercase'}}>
          ...
          <div style={{display:'flex',gap:'clamp(12px,2vw,24px)',flexWrap:'wrap'}}>
            {[{l:'Behance',h:'https://behance.net/abhijeeth-subhash'},{l:'Dribbble',h:'https://dribbble.com/Artificer_666'},{l:'Instagram',h:'https://instagram.com/wonderartmedia'},{l:'LinkedIn',h:'https://linkedin.com/in/abhijeethsubhash'}].map(({l,h}) => (
              <a key={l} href={h} target="_blank" rel="noopener noreferrer" className="hover-line"
                style={{fontFamily:'var(--font-mono)',fontSize:'10px',color:'var(--dim)',letterSpacing:'1px'}}>{l}</a>
            ))}
          </div>
        </div>
      </footer>
