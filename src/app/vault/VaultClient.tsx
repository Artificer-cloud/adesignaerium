...
function getAspectRatio(w:number,h:number):string {
  const r = h/w
  if (r < 0.6)  return '16/9'
  if (r < 0.85) return '4/3'
...
function GalleryCard({ item, index, onOpen }: { item:GalleryImage; index:number; onOpen:()=>void }) {
  ...
  return (
    <div ref={ref} onClick={onOpen} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ position:'relative', overflow:'hidden', cursor:'pointer', background:'var(--surface)', aspectRatio:ratio, borderRadius:'4px', opacity:vis?1:0, transform:vis?'translateY(0)':'translateY(28px)', transition:`opacity .7s cubic-bezier(.23,1,.32,1) ${(index%6)*.06}s, transform .7s cubic-bezier(.23,1,.32,1) ${(index%6)*.06}s` }}>
      <Image src={item.src} alt={item.alt} fill sizes="(max-width:640px) 50vw,(max-width:1024px) 33vw,25vw"
        style={{ objectFit:'cover', transition:'transform .7s cubic-bezier(.23,1,.32,1)', transform:hov?'scale(1.07)':'scale(1)' }} onError={()=>{}}/>
      <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top,rgba(8,8,8,0.92) 0%,rgba(8,8,8,0.3) 50%,rgba(8,8,8,0.05) 100%)', opacity:hov?1:0, transition:'opacity .5s cubic-bezier(.23,1,.32,1)' }}/>
      <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'clamp(14px,2vw,22px)', transform:hov?'translateY(0)':'translateY(12px)', opacity:hov?1:0, transition:'all .4s cubic-bezier(.23,1,.32,1)' }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:'8px', letterSpacing:'3px', color:'var(--orange)', display:'block', marginBottom:'5px', textTransform:'uppercase' }}>{item.category}</span>
        <p style={{ fontFamily:'Clash Display,Arial Black,sans-serif', fontWeight:600, fontSize:'clamp(13px,1.6vw,18px)', color:'var(--bone)', letterSpacing:'-0.3px', lineHeight:1.1, margin:0 }}>{item.alt}</p>
        <div style={{ width:'20px', height:'2px', background:'var(--orange)', marginTop:'8px', borderRadius:'2px' }}/>
      </div>
      <div style={{ position:'absolute', top:'12px', left:'12px', display:'flex', alignItems:'center', gap:'5px', opacity:hov?0:1, transition:'opacity .3s' }}>
      ...
      <div style={{ position:'absolute', top:'12px', right:'12px', width:'30px', height:'30px', borderRadius:'50%', background:'var(--orange)', display:'flex', alignItems:'center', justifyContent:'center', opacity:hov?1:0, transform:hov?'scale(1) rotate(0deg)':'scale(0.5) rotate(-45deg)', transition:'all .4s cubic-bezier(.23,1,.32,1)' }}>
...
export default function VaultClient({ images }: { images:GalleryImage[] }) {
  ...
  useEffect(() => {
    ...
    const fn = (e:KeyboardEvent) => {
      ...
      else if (e.key==='ArrowRight') { const idx=filtered.findIndex(i=>i.id===lightbox.id); setLightbox(filtered[(idx+1)%filtered.length]) }
      else if (e.key==='ArrowLeft')  { const idx=filtered.findIndex(i=>i.id===lightbox.id); setLightbox(filtered[(idx-1+filtered.length)%filtered.length]) }
    }
  ...
  return (
    <main style={{ paddingTop:'80px', minHeight:'100vh' }}>
      <section style={{ padding:'clamp(40px,6vh,80px) clamp(20px,6vw,80px) clamp(24px,4vh,40px)', maxWidth:'1600px', margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', flexWrap:'wrap', gap:'24px', marginBottom:'32px' }}>
          ...
          <div style={{ textAlign:'right' }}>
            ...
            <span style={{ fontFamily:'var(--font-mono)', fontSize:'10px', color:'var(--dim)', letterSpacing:'1px' }}>{filtered.length} images</span>
          </div>
        </div>
        <div style={{ display:'flex', flexWrap:'wrap', gap:'8px' }}>
          {allCats.map(cat=>(
            <button key={cat} onClick={()=>setActive(cat)} style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'2px', textTransform:'uppercase', padding:'8px 20px', borderRadius:'100px', cursor:'pointer', transition:'all .3s cubic-bezier(.23,1,.32,1)', background:active===cat?'var(--orange)':'transparent', color:active===cat?'var(--ink)':'var(--muted)', border:active===cat?'1px solid var(--orange)':'1px solid var(--border)' }}>{cat}</button>
          ))}
        </div>
      </section>
        ...
        <div onClick={()=>setLightbox(null)} style={{ position:'fixed', inset:0, zIndex:9990, background:'rgba(8,8,8,0.97)', display:'flex', alignItems:'center', justifyContent:'center', padding:'clamp(20px,4vw,60px)' }}>
          ...
          <div onClick={e=>e.stopPropagation()} style={{ position:'relative', maxWidth:'min(90vw,1000px)', maxHeight:'90vh', borderRadius:'4px', overflow:'hidden' }}>
            <Image src={lightbox.src} alt={lightbox.alt} width={lightbox.width} height={lightbox.height}
              style={{ width:'100%', height:'auto', maxHeight:'85vh', objectFit:'contain', display:'block' }} onError={()=>{}}/>
      ...
      <style>{`
        .vault-grid { columns:4; column-gap:clamp(6px,0.8vw,10px); }
        .vault-grid>div { break-inside:avoid; margin-bottom:clamp(6px,0.8vw,10px); display:block; }
