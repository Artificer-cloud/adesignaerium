...
const TIMELINE = [
  { year:'2023–Present', role:'Graphic Designer / Creative Director', place:'Krossover Gifts Trading LLC — Dubai, UAE', pts:['Full creative direction for 100+ B2B corporate gifting clients','E-commerce UI/UX: kross-over.net & shanghaigifts.net','Built Ecora, Maison Valér & Sipple brand universes from scratch','Reels, product videos, lifestyle imagery, print catalogues','100% on-time delivery across 50+ concurrent projects'] },
  { year:'2020–2022',    role:'Graphic Design Artist',                 place:'Creative Dreams Design — Kerala, India',          pts:['Branding, UI/UX & digital illustration for 30+ clients','Industries: tech, retail, hospitality','End-to-end Adobe Creative Suite execution'] },
  { year:'2019–2020',    role:'Graphic Design Faculty',                place:'KELTRON Advanced Studies — Trivandrum, Kerala',    pts:['Trained 100+ designers — 90% placement rate in design roles','Developed curriculum: UI/UX, branding, Adobe Suite','Led live client projects and industry workshops'] },
  ...
  return (
    <main style={{ paddingTop:'80px', background:'var(--ink)', minHeight:'100vh' }}>
      <section style={{ padding:'clamp(48px,8vh,100px) clamp(20px,6vw,80px) 0', maxWidth:'1400px', margin:'0 auto' }}>
        <span style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'3px', color:'var(--orange)', display:'block', marginBottom:'16px', opacity:mounted?1:0, transition:'opacity .6s .1s' }}>✦ THE PERSON</span>
        <h1 style={{ fontFamily:'Clash Display,Arial Black,sans-serif', fontWeight:700, fontSize:'clamp(52px,10vw,130px)', letterSpacing:'-4px', color:'var(--bone)', lineHeight:.86, opacity:mounted?1:0, transform:mounted?'translateY(0)':'translateY(32px)', transition:'all .9s cubic-bezier(.23,1,.32,1) .2s' }}>
          ABOUT<br/><span style={{ fontFamily:'Cormorant Garamond,Georgia,serif', fontStyle:'italic', fontWeight:600, color:'var(--orange)' }}>Abhijeeth.</span>
        </h1>
      </section>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'clamp(260px,38%,500px) 1fr', gap:'clamp(32px,5vw,80px)', alignItems:'start' }}>
          <div className="reveal-left" style={{ position:'relative' }}>
            <div style={{ position:'relative', width:'100%', aspectRatio:'4/5', borderRadius:'4px', overflow:'hidden', background:'var(--surface)' }}>
              <Image src="/images/about-photo.webp" alt="Abhijeeth Subhash — Senior Creative Designer Dubai" fill style={{ objectFit:'cover', objectPosition:'center top' }} onError={()=>{}}/>
              <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'6px', pointerEvents:'none', zIndex:1 }}>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'2px', color:'rgba(255,255,255,0.15)', textAlign:'center', lineHeight:2 }}>YOUR PHOTO HERE<br/><span style={{fontSize:'9px'}}>about-photo.webp · 800×1000px</span></span>
              </div>
            ...
            <p style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'2px', color:'var(--muted)', marginTop:'12px' }}>DUBAI, UAE · 2026</p>
          </div>
          ...
          <div className="reveal-right">
            <p style={{ fontFamily:'Cormorant Garamond,Georgia,serif', fontStyle:'italic', fontWeight:500, fontSize:'clamp(20px,2.5vw,32px)', color:'var(--bone)', lineHeight:1.5, marginBottom:'28px' }}>
              I turn concepts into visual worlds. Born in Kerala, India. Currently building brands in <span style={{color:'var(--orange)'}}>Dubai, UAE.</span>
            </p>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'8px', marginBottom:'40px' }}>
              {[{l:'+971 52 677 6884',h:'tel:+971526776884'},{l:'abhijeethpiyush4@gmail.com',h:'mailto:abhijeethpiyush4@gmail.com'},{l:'LinkedIn',h:'https://linkedin.com/in/abhijeethsubhash'},{l:'Behance',h:'https://behance.net/abhijeeth-subhash'},{l:'Dribbble',h:'https://dribbble.com/Artificer_666'},{l:'@wonderartmedia',h:'https://instagram.com/wonderartmedia'}].map(({l,h})=>(
                <a key={l} href={h} target={h.startsWith('http')?'_blank':undefined} rel={h.startsWith('http')?'noopener noreferrer':undefined} className="hover-line"
                  style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'1px', color:'var(--muted)', border:'1px solid var(--border)', padding:'7px 14px', borderRadius:'2px', display:'inline-block' }}>{l} ↗</a>
              ))}
            </div>
      ...
      <section style={{ background:'var(--surface)', borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)' }}>
        <div style={{ padding:'clamp(48px,7vh,80px) clamp(20px,6vw,80px)', maxWidth:'1400px', margin:'0 auto' }}>
          <span className="reveal" style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'3px', color:'var(--orange)', display:'block', marginBottom:'40px' }}>✦ SKILLS & EXPERTISE</span>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,220px),1fr))', gap:'1px', background:'var(--border)' }}>
            {SKILLS_LIST.map(({category,items})=>(
              <div key={category} className="reveal" style={{ background:'var(--surface)', padding:'clamp(20px,2.5vw,32px)' }}>
                <div style={{ fontFamily:'Clash Display,Arial Black,sans-serif', fontWeight:700, fontSize:'13px', letterSpacing:'2px', color:'var(--orange)', marginBottom:'16px', textTransform:'uppercase' }}>{category}</div>
      ...
      <section style={{ padding:'clamp(48px,7vh,100px) clamp(20px,6vw,80px)', maxWidth:'1400px', margin:'0 auto' }}>
        <span className="reveal" style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'3px', color:'var(--orange)', display:'block', marginBottom:'48px' }}>✦ THE JOURNEY</span>
        <div style={{ position:'relative' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'clamp(36px,5vh,56px)', paddingLeft:'40px' }}>
            {TIMELINE.map(({year,role,place,pts},i)=>(
              <div key={year} className="reveal" style={{ position:'relative', transitionDelay:`${i*.1}s` }}>
      ...
      <section style={{ background:'var(--surface)', borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)' }}>
        <div style={{ padding:'clamp(48px,7vh,80px) clamp(20px,6vw,80px)', maxWidth:'1400px', margin:'0 auto' }}>
          <span className="reveal" style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'3px', color:'var(--orange)', display:'block', marginBottom:'24px' }}>✦ AI PHILOSOPHY</span>
          ...
          <div className="reveal" style={{ display:'flex', flexWrap:'wrap', gap:'8px' }}>
            {['Midjourney','Sora','Higgsfield','OpenAI / ChatGPT','Gemini','Nano Banana Pro','Adobe Firefly'].map(tool=>(
              <span key={tool} style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'1px', color:'var(--bone)', background:'rgba(255,77,0,0.07)', border:'1px solid rgba(255,77,0,0.18)', padding:'6px 14px', borderRadius:'2px' }}>{tool}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding:'clamp(60px,8vh,100px) clamp(20px,6vw,80px)', maxWidth:'1400px', margin:'0 auto', textAlign:'center' }}>
        <h2 className="reveal" style={{ fontFamily:'Clash Display,Arial Black,sans-serif', fontWeight:700, fontSize:'clamp(36px,7vw,80px)', letterSpacing:'-3px', color:'var(--bone)', lineHeight:.9, marginBottom:'16px' }}>LET&apos;S BUILD</h2>
        <h2 className="reveal" style={{ fontFamily:'Cormorant Garamond,Georgia,serif', fontStyle:'italic', fontWeight:600, fontSize:'clamp(36px,7vw,80px)', letterSpacing:'-2px', color:'var(--orange)', lineHeight:.9, marginBottom:'40px' }}>Something great.</h2>
        <div className="reveal" style={{ display:'flex', flexWrap:'wrap', gap:'12px', justifyContent:'center' }}>
          <a href="mailto:abhijeethpiyush4@gmail.com" style={{ fontFamily:'Clash Display,Arial Black,sans-serif', fontWeight:600, fontSize:'12px', letterSpacing:'2px', textTransform:'uppercase', background:'var(--orange)', color:'var(--ink)', padding:'14px 40px', borderRadius:'2px', textDecoration:'none', display:'inline-block' }}>Email Me →</a>
          <Link href="/work" style={{ fontFamily:'var(--font-mono)', fontSize:'11px', letterSpacing:'2px', textTransform:'uppercase', border:'1px solid var(--border)', color:'var(--bone)', padding:'14px 40px', borderRadius:'2px', display:'inline-block', textDecoration:'none' }}>View Work</Link>
          <a href="https://wa.me/971526776884" target="_blank" rel="noopener noreferrer" style={{ fontFamily:'var(--font-mono)', fontSize:'11px', letterSpacing:'2px', textTransform:'uppercase', border:'1px solid var(--border)', color:'var(--bone)', padding:'14px 40px', borderRadius:'2px', display:'inline-block', textDecoration:'none' }}>WhatsApp ↗</a>
        </div>
      </section>
    </main>
  )
